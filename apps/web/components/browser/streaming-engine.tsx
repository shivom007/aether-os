...
import { reconstructShards, fetchMinimumShards } from "@/lib/erasure"
  ...
  useEffect(() => {
    ...
    const getDecryptedChunk = async (inodeId: string, chunkIndex: number): Promise<Uint8Array> => {
      ...
      const promise = (async () => {
        ...
        const dataShards = targetShards[0].data_shards || 10
        const parityShards = targetShards[0].parity_shards || 4

        // 4. Fetch exactly `dataShards` shards intelligently
        ...
        }, undefined, dataShards, parityShards)
