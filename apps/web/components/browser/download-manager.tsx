...
import { reconstructShards, fetchMinimumShards, DEFAULT_DATA_SHARDS as DATA_SHARDS } from "@/lib/erasure"
    ...
    const processQueue = async () => {
      ...
      try {
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          if (chunkShards && chunkShards.length > 0) {
            globalTotalShards += (chunkShards[0].data_shards || DATA_SHARDS)
          }
        }
        ...
        for (let i = 0; i < totalChunks; i++) {
          const chunkShards = chunksMap.get(i)
          if (!chunkShards || chunkShards.length === 0) throw new Error(`Chunk ${i} is entirely missing from the database.`)
          
          const dataShards = chunkShards[0].data_shards || DATA_SHARDS
          const parityShards = chunkShards[0].parity_shards || (chunkShards.length - dataShards)
          ...
          const fetchedShards = await fetchMinimumShards(
            ...
            () => {
              ...
              updateTask(nextTask.id, { 
                progress: Math.min(100, (globalDownloadedShards / globalTotalShards) * 100),
                downloadedShards: globalDownloadedShards,
                totalShards: globalTotalShards
              })
            },
            dataShards,
            parityShards
          ...
          const reconstructed = await reconstructShards(fetchedShards, originalSize, dataShards, parityShards)
