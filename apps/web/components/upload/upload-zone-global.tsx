  ...
  const processFile = async (
    ...
    try {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        // UPLOAD ALL SHARDS CONCURRENTLY WITH RETRIES
        ...
        for (let idxShard = 0; idxShard < totalShards; idxShard++) {
          ...
          const uploadWithRetry = async (attempt = 1): Promise<void> => {
            ...
            // MUST append shardId FIRST so the Go server knows which shard to stream to
            ...
            try {
              ...
              if (attempt < 3) {
                console.warn(`Shard upload failed, retrying (${attempt}/3)...`, err);
