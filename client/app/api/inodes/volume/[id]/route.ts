...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const fsData = await goFetch<{
      files: Array<{ id: number; name: string; size: number; mimeType: string; thumbnail?: string; createdAt: string; updatedAt: string }> | null
      folders: Array<{ id: number; name: string; createdAt: string; updatedAt: string }> | null
    }>(`/fs?volumeId=${id}`, { token })
